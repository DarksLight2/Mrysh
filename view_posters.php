...
<div id="view_posters_box">
    <?php
    $query = $Dialogs->select()->where(['userID' => User::userData()['id']])->orWhere(['companionID' => User::userData()['id']])->limit($start_select.', '.$count_dialogs_on_page)->execute();
        foreach ($query->data as $dialog) {
...
<?php
            ...
            echo '<div class="ml10 mt5 mr5">У вас нету начатых переписок</div>';
        }
    }
    ?>
</div>
